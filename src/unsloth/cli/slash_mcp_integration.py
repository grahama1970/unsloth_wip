"""Slash command and MCP integration for Unsloth CLI.

This module generates slash commands and MCP configuration for the Unsloth CLI,
making all commands (including RunPod) accessible to Claude and other agents.
"""
...
def add_generation_commands(cli: click.Group):
    ...
    def generate_mcp():
        ...
        # Print usage instructions
        click.echo("\n📡 To use with MCP:")
        click.echo(f"1. Start the MCP server: unsloth-mcp")
        click.echo(f"2. Add the config to your MCP client")
        click.echo(f"3. Tools will be available with prefix 'unsloth_'")
